...
def test_qwen38_hardware_harness_is_static_safe_and_metrics_only():
    ...
    assert all(name in source for name in ("CONTEXT_8K","CONTEXT_16K","CONTEXT_24K","CONTEXT_32K"))
    assert "max_qualified_context_tokens" in source and "vision_qualified" in source
