...
WORKER = ROOT / "control-plane/scripts/whisper-direct-work-worker.py"
...
def base_result(started_at: str, final_status: str, reason: str) -> dict[str, Any]:
    return {
        "schema_version": "0.1",
...
def fail_before_model(output_dir: Path, started_at: str, reason: str) -> int:
    result = {
        **base_result(started_at, "FUNCTIONAL_FAIL", reason),
        "model_started": False,
        "cleanup_ok": True,
    }
...
def main() -> int:
    ...
    inputs = {
        "zh": args.zh.expanduser().resolve(),
        "en": args.en.expanduser().resolve(),
        "noisy": args.noisy.expanduser().resolve(),
    }

    for label, path in inputs.items():
        if not path.is_file():
            return fail_before_model(output_dir, started_at, f"INPUT_MISSING:{label}")

    if not MODEL_PATH.is_dir():
        return fail_before_model(output_dir, started_at, "MODEL_PATH_MISSING")
    if not AUDIO_PYTHON.is_file():
        return fail_before_model(output_dir, started_at, "AUDIO_VENV_PYTHON_MISSING")
    if not WORKER.is_file():
        return fail_before_model(output_dir, started_at, "WORKER_SCRIPT_MISSING")
    if shutil.which("ffmpeg") is None or shutil.which("ffprobe") is None:
        return fail_before_model(output_dir, started_at, "FFMPEG_OR_FFPROBE_MISSING")
    ...
    if import_check.returncode != 0 or "MLX_WHISPER_OK" not in import_check.stdout:
        (output_dir / "dependency-check.stderr.txt").write_text(
            import_check.stderr[-4000:],
            encoding="utf-8",
        )
        return fail_before_model(output_dir, started_at, "MLX_WHISPER_IMPORT_FAILED")

    manifest = WorkloadManifestProbe(ports=(), top_n=50).capture(
    ...
    if not preflight.allowed:
        result = {
            **base_result(
                started_at,
                "RESOURCE_BLOCKED",
                f"RESOURCE_PREFLIGHT_DENIED:{preflight.reason}",
            ),
            "model_started": False,
            "cleanup_ok": True,
        }
    ...
    elif worker_returncode not in {0, None} and functional_reason == "UNCLASSIFIED":
    ...
    if not worker_json.is_file():
        ...
        result = {
            **base_result(started_at, final_status, functional_reason),
            "model_started": True,
            "cleanup_ok": cleanup_ok,
            "warning_observed": bool(monitor and monitor.warning_observed),
            "peak_swap_delta_gib": max(
                (x["swap_delta_gib"] for x in (monitor.samples if monitor else [])),
                default=0.0,
            ),
        }
        write_json(output_dir / "result.json", result)
    ...
    if automatic_ok and not args.non_interactive:
        ...
        for label in ("zh", "en", "noisy"):
            preview = (output_dir / f"{label}.transcript.txt").read_text(
                encoding="utf-8"
            )[:2200]
            print()
            print(f"--- {label.upper()} ---")
            ...
            print()
            meaning = yes_no(
                f"[{label}] Meaning usable for clip selection? [y/n]: "
            )
            names_numbers = yes_no(
                f"[{label}] Names/numbers acceptable without wholesale retranscription? [y/n]: "
            )
            timestamps = yes_no(
                f"[{label}] Timestamps usable for seeking/cutting? [y/n]: "
            )
            owner_review[label] = {
    ...
    peak_swap_delta = max(
        (x["swap_delta_gib"] for x in samples),
        default=0.0,
    )
    peak_swap_used = max(
        (x["swap_used_gib"] for x in samples),
        default=preflight.snapshot.swap_used_gib,
    )
    min_reclaimable = min(
        (
            x["reclaimable_gib"]
            for x in samples
            if x["reclaimable_gib"] is not None
        ),
        default=preflight.snapshot.reclaimable_gib,
    )
    ...
    result = {
        **base_result(started_at, final_status, reason),
        ...
        "min_reclaimable_gib": (
            None
            if min_reclaimable is None
            else round(float(min_reclaimable), 4)
        ),
    ...
    safe_lines = [
        ...
        (
            f"- Minimum reclaimable: `{float(min_reclaimable):.3f} GiB`"
            if min_reclaimable is not None
            else "- Minimum reclaimable: `unknown`"
        ),
        "",
        "## Clip metrics",
        "",
    ...
    for label in ("zh", "en", "noisy"):
        ...
        safe_lines.append(
            f"| `{label}` | `{row['input_sha256']}` | "
            f"{row['duration_seconds']} | {row['runtime_seconds']} | "
            f"{row['real_time_factor']} | `{row['detected_language']}` | "
            f"{row['segment_count']} | `{row['timestamp_structure_ok']}` |"
        )
    ...
    if owner_review:
        for label in ("zh", "en", "noisy"):
            ...
            safe_lines.append(
                f"- `{label}` meaning={review['meaning_usable']}, "
                f"names_numbers={review['names_numbers_acceptable']}, "
                f"timestamps={review['timestamps_usable']}"
            )
    else:
    ...
    safe_lines.extend(
        [
            "",
            "Private media and raw transcripts remain outside Git. "
            "Only this safe summary, hashes and metrics are intended for synchronization.",
            "",
        ]
    )
    safe_summary = "\n".join(safe_lines)
    (output_dir / "git-summary.md").write_text(
        safe_summary,
        encoding="utf-8",
    )
