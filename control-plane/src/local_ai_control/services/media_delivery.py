...
SECRET_PATTERN = re.compile(r"(?i)(token|secret|password|credential|authorization|cookie|private[_ -]?key)")
...
class MediaPublisher:
    ...
    def publish(self, workspace: MediaWorkspace, *, push=True) -> dict:
        ...
        self._preflight(); workspace.transition(MediaWorkflowState.PUBLISH_PENDING,reason="fixed repository publish")
        target=self.repository/job.task_slug
        if target.exists(): raise MediaWorkflowError("MEDIA_PUBLISH_TARGET_EXISTS")
        for relative in ("source","generated","output","metadata"):
            (target/relative).mkdir(mode=0o755,parents=True,exist_ok=True)
        (target/"README.md").write_text(f"# {job.task_name}\n\nLocally generated and Owner-approved media product.\n","utf-8")
        shutil.copyfile(source,target/"output"/"final.mp4",follow_symlinks=False)
        for relative in ("requirements.md","production_brief.md","script.txt","scene_plan.json","prompt_pack.json","source_evidence.json"):
            origin=workspace.path/relative
            if origin.is_file() and not origin.is_symlink(): shutil.copyfile(origin,target/("source" if relative in {"requirements.md","production_brief.md","source_evidence.json"} else "generated")/relative,follow_symlinks=False)
        ...
        if push: self._git("push","origin","HEAD")
    ...
    def _validate_metadata(metadata: dict) -> None:
        ...
        if SECRET_PATTERN.search(encoded) or "/Users/" in encoded: raise MediaWorkflowError("MEDIA_PUBLISH_METADATA_SENSITIVE")
