...
from local_ai_control.services.context_budget import ContextBudgetError, ContextBudgetManager
...
class ChatService:
    ...
    def reply(self, identity, session_id, message):
        ...
        max_output_tokens = output_budget(message)

        try:
            budget = self.context_budget_manager.for_provider(
                self.provider,
            ...
            messages = self.repository.recent_messages(
                identity,
                session_id,
                self.assembler.policy.history_fetch_count,
            )
            ...
            memories = self.repository.list_memories(
                identity,
                limit=self.assembler.policy.memory_count,
            )
            ...
            context = self.assembler.assemble(
                ...
                summary,
                memories,
                token_budget=budget.input_budget_tokens,
            )

        except ContextBudgetError:
            return ChatResult(
                "本次输入超过当前模型已验证的上下文预算，请缩短单次输入或拆分任务。",
                False,
                "context_limit",
                None,
                max_output_tokens,
