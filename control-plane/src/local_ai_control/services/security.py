...
class SecretFirewall:
    ...
    _patterns = (
        ...
        ("api_key", re.compile(r"\b(?:api[_ -]?key|access[_ -]?token)\s*[:=]\s*[A-Za-z0-9._-]{16,}\b", re.I)),
        ("authorization_header", re.compile(r"\bAuthorization\s*:\s*(?:Basic|Bearer|Token)\s+[A-Za-z0-9+/=._-]{12,}\b", re.I)),
        ("session_cookie", re.compile(r"\b(?:session|sessionid|cookie)\s*[:=]\s*[A-Za-z0-9%._-]{16,}\b", re.I)),
