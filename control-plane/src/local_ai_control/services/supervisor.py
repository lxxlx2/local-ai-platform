...
from .supervisor_codex import (
    REAL_CODEX_EXECUTION_ENABLED,
    REAL_CODEX_RESUMABLE,
    CodexCapability,
...
class WorkflowSupervisor:
    def __init__(
        ...
        runners: Mapping[WorkflowStage, StageRunner],
    ...
    def _require_lease(self) -> None:
        ...
        try:
            self.repository.record_event(
                None,
                "LEASE_LOST",
                payload={"consumer_stopped": True},
            )
        except Exception:
            pass
        raise LeaseLostError("supervisor singleton lease lost")
    ...
    def pause(self, job_id: str, owner_id: str | None = None) -> WorkflowJob:
        ...
        if job.status in TERMINAL_JOB_STATUSES or job.status is JobStatus.BLOCKED:
            return job
        if job.resume_state in {"PAUSED", "PAUSE_REQUESTED"}:
            return job
    ...
    def retry(self, job_id: str, owner_id: str | None = None) -> WorkflowJob:
        ...
        self.repository.record_event(
            ...
            job.current_stage,
            {"manual": True},
            f"retry:{job_id}:{job.current_stage.value}:{job.attempt}",
        )
        return job
        ...
        for job in self.repository.list_jobs(limit=200):
            ...
            with self.repository.db:
                self.repository.db.execute(
                    "UPDATE supervisor_stage_runs SET status='INTERRUPTED',completed_at=?,"
                    "error='PROCESS_INTERRUPTED' WHERE job_id=? AND status='RUNNING'",
                    (utc_now(), job.job_id),
                )
            ...
            if job.current_stage in SAFE_RECOVERY_STAGES and attempts < job.max_attempts_per_stage:
                self.repository.update_job(
                    job.job_id,
                    status=JobStatus.QUEUED,
                    resume_state="INTERRUPTED_SAFE_RETRY",
    ...
    def _schedule_failure(self, job: WorkflowJob, result: StageResult) -> WorkflowJob:
        ...
        if result.status is StageResultStatus.BLOCKED or job.current_stage in {
            WorkflowStage.SECURITY,
            WorkflowStage.GIT_GATE,
        }:
    ...
    def _handle_review_result(self, job: WorkflowJob, result: StageResult) -> WorkflowJob | None:
        ...
        if result.status is not StageResultStatus.FAIL:
            return None
        ...
        if result.review_findings:
            ...
            if review_round >= job.max_review_rounds:
                updated = self.repository.update_job(
                    ...
                    resume_state="MAX_REVIEW_ROUNDS",
                    last_error="REVIEW_FINDINGS_MISSING",
                )
                self.repository.record_event(
                    job.job_id,
                    "STAGE_FAILED",
    ...
    def _execute_job(self, job: WorkflowJob) -> WorkflowJob:
        ...
        try:
            ...
            result = StageResult.failed(
                "Stage runner raised a bounded error",
                type(error).__name__,
            )
        ...
        review_handled = self._handle_review_result(job, result)
        if review_handled is not None:
            return review_handled
        if result.status is not StageResultStatus.PASS:
            return self._schedule_failure(job, result)
        if job.current_stage is WorkflowStage.REVISION and job.review_round > 0:
        ...
        updated = self.repository.update_job(
            job.job_id,
            status=JobStatus.WAITING if pause_requested else JobStatus.QUEUED,
            current_stage=next_stage,
            attempt=0,
            resume_state="PAUSED" if pause_requested else None,
            last_error=None,
            next_retry_at=None,
        )
        self.repository.record_event(
    ...
    def run_job_once(self, job_id: str) -> WorkflowJob:
        self._require_lease()
    ...
    def run_until_terminal(self, job_id: str, max_transitions: int = 50) -> WorkflowJob:
        transitions = 0
        while transitions < max_transitions:
            job = self.status(job_id)
            if (
                job.status in TERMINAL_JOB_STATUSES
                or job.status is JobStatus.BLOCKED
                or job.resume_state == "PAUSED"
            ):
                return job
            if (
                job.status is JobStatus.WAITING
                and job.resume_state == "RETRY_SCHEDULED"
                and (job.next_retry_at or 0) > time.time()
            ):
                return job
            before_runs = len(self.repository.latest_stage_runs(job_id))
            before = (
                job.status,
                job.current_stage,
                job.attempt,
            ...
            self.run_job_once(job_id)
            after = self.status(job_id)
            after_runs = len(self.repository.latest_stage_runs(job_id))
            snapshot = (
                after.status,
                after.current_stage,
                after.attempt,
            ...
            progress = max(after_runs - before_runs, 1 if snapshot != before else 0)
            if progress == 0:
                return after
        ...
        job = self.repository.update_job(
            job_id,
            status=JobStatus.BLOCKED,
            resume_state="TRANSITION_LIMIT",
            last_error="TRANSITION_LIMIT",
        )
        self.repository.record_event(
            job_id,
            "STAGE_FAILED",
...
def default_demo_runners(real_validation: bool = True) -> dict[WorkflowStage, StageRunner]:
    validation: StageRunner = (
        LocalValidationRunner()
        if real_validation
        else StaticPassRunner("Mock local validation passed")
    )
    return {
