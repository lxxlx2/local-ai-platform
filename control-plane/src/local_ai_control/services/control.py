...
from local_ai_control.domain.state import TaskState,transition
...
class ControlPlane:
 ...
 def migrate(self):
  ...
  self.db.execute("INSERT OR IGNORE INTO model_registry VALUES ('qwen36_fast','Qwen3.6','FAST',1,8192)");self.db.commit()
 ...
 def set_state(self,i,new):
  old=self.db.execute('SELECT state FROM tasks WHERE id=?',(i,)).fetchone()['state'];transition(old,new);self.db.execute('UPDATE tasks SET state=?,updated_at=? WHERE id=?',(new,now(),i));self.audit('TASK_STATE',{'task_id':i,'from':old,'to':new});self.db.commit()
 ...
 def decide(self,i,owner,version,decision):
  ...
  state='APPROVED' if decision=='approve' else 'REJECTED';self.db.execute('UPDATE approvals SET state=?,version=?,updated_at=? WHERE id=?',(state,version+1,now(),i));self.audit('APPROVAL_'+state,{'approval_id':i});self.db.commit();return state
