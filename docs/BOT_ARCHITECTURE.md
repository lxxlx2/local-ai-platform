...
The strict versioned model registry and runtime provider factory make qualified Qwen3.8 MAIN the normal chat candidate, with a 16K machine envelope. Its localhost-only sidecar also handles Owner-private image understanding. Qwen3.6 remains the validated FAST/FALLBACK model for `/fast` and health/resource fallback. The lifecycle never permits both heavy runtimes to report healthy at once and restores MAIN after FAST use. Neither model is a Codex replacement because autonomous coding-agent acceptance remains separate. Open WebUI remains optional debug UI and is not installed.
