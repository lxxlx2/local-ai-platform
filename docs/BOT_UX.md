...
Capability questions are deterministic product responses, not free-form model descriptions. They disclose the active model and local backend, while respecting Owner/Public scope. Code blocks are presented as readable indented code without raw Markdown fence markers; inline code uses a visible `〔代码：…〕` boundary.
