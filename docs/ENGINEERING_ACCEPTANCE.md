...
Workflow Supervisor candidates add deterministic transition, duplicate-resume, interrupted-stage, attempt/round ceiling, singleton-lock, command allowlist, path-boundary, secret-audit, storage-retention, and Owner-isolation gates. A mock Review FAIL must flow automatically through Revision and re-validation before Review PASS. A Supervisor self-test cannot replace the independent review gate; experimental feature branches remain `REVIEW_PENDING`.
